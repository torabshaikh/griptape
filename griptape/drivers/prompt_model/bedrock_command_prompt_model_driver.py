...
from griptape.tokenizers import BedrockCommandTokenizer
from griptape.drivers import AmazonBedrockPromptDriver


@define
class BedrockCommandPromptModelDriver(BasePromptModelDriver):
    model: str = field(default="amazon.titan-tg1-large", kw_only=True)
    top_p: float = field(default=0.9, kw_only=True)
    _tokenizer: BedrockCommandTokenizer = field(default=None, kw_only=True)

    @property
    def tokenizer(self) -> BedrockCommandTokenizer:
        ...
        Returns:
            BedrockCommandTokenizer: The tokenizer for this driver.
        """
        if self._tokenizer:
            return self._tokenizer
        else:
            if isinstance(self.prompt_driver, AmazonBedrockPromptDriver):
                self._tokenizer = BedrockCommandTokenizer(model=self.model, session=self.prompt_driver.session)
                return self._tokenizer
            else:
